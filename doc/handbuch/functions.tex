% ==============================================================================
%  $RCSfile: functions.tex,v $, $Revision: 1.10 $
%  $Date: 2003/08/09 14:28:28 $
%  $Author: birdy $
%
%  Description: 
%
...
\index{Anzeigefenster}
 ...
 in der Vis Pane entsprechend angepasst (siehe auch Funktion 
 \ref{Verschieben des sichtbaren Anzeigeinhaltes über die Minimap}).
  ...
  \subsection{Selektionsauswahlliste} \label{Selektionsauswahlliste}
    ...
    \begin{enumerate}
     ...
     \end{itemize}
      \item Show Selection\\
            Mit dieser Funktion können ausgeblendete Selektionen und alle dazugehörigen
            Fenster-Knoten und Fenster-Kanten wieder eingeblendet
	    werden.
	 \begin{itemize}
	    \item{Der Benutzer führt einen Rechtsklick mit der Maus auf
            eine ausgeblendete Selektion in der Selektionsauswahlliste (siehe \ref{Selektionsauswahlliste})
	    durch und wählt im Popup-Menü den Eintrag \gq{Show Selection} aus.}
         \end{itemize}
      \item Show all\\
        Mit dieser Funktion können alle ausgeblendeten Fenster-Knoten
	...
	\begin{itemize}
	  \item{Der Benutzer führt einen Rechtsklick mit der Maus auf
                ...
                (siehe \ref{Selektionsauswahlliste}) durch 
                 und wählt im Popup-Menü den Eintrag \gq{Show all} aus.}
	\end{itemize}
      \item Hide Selection\\
            Mit dieser Funktion können ausgeblendete Selektionen und alle dazugehörigen
            Fenster-Knoten und Fenster-Kanten wieder eingeblendet
	 ...
	 \begin{itemize}
	    \item{Der Benutzer führt einen Rechtsklick mit der Maus auf
            eine eingeblendete Selektion in der Selektionsauswahlliste (siehe \ref{Selektionsauswahlliste})
	    durch und wählt im Popup-Menü den Eintrag \gq{Hide Selection} aus.}
         \end{itemize}
	  ...
	  Hat der Benutzer einen Layoutalgorithmus während der
	...
	Der Benutzer startet die Funktion mit einem Rechtsklick 
      ...
      \item Zoom To Make Selection Fill Window\\
	 ...
	 sichtbar ist. Die Detailstufe (siehe \ref{Visualization Detailstufen})
	 wurde ggf. automatisch angepasst.
      \item Copy Selection\\
          Diese Funktion dient zum Kopieren von Selektionen innerhalb eines Anzeigefensters.
	  \begin{itemize}
	   \item{Der Benutzer startet die Funktion durch Rechtsklick auf die zu kopierende
	   ...
	   \item{GIANT öffnet einen Texteingabedialog. Der Benutzer gibt dort einen
	         zulässigen Namen für die neue Selektion 
                 ein und bestätigt mit OK.}
           \item{GIANT kopiert die Quell-Selektion und legt eine neue Selektion an.}
	  \end{itemize}
      \item Copy Selection Keeping Existing Layout\\
          Mit dieser Funktion kann eine Selektion aus dem Quell-Anzeigefenster in ein
          anderes Anzeigefenster unter Beibehaltung des Layouts kopiert werden.\\
	  Die Position von Fenster-Knoten, die vor dem Einfügen bereits im
          Ziel-Anzeigefenster vorhanden waren, bleibt unverändert.
	  Die kopierte Selektion existiert nach Ausführung der Funktion auch im
	  Ziel-Anzeigefenster als Selektion, sie wird dort zur Standard-Selektion.
	  Der alte Inhalt der Standard-Selektion wird überschrieben.
          \begin{itemize}
	    \item{Der Benutzer startet die Funktion über das Popup-Menü
                  der Selektionsauswahlliste (siehe \ref{Selektionsauswahlliste}), 
                  indem er dort den Menüeintrag \gq{Copy Selection Keeping Existing
		  Layout} auswählt.}
            \item{Der Benutzer wählt das gewünschte Ziel-Anzeigefenster aus.
	    ...
	    \item{GIANT kopiert die Fenster-Knoten und Fenster-Kanten der 
		  ...
		  Die Position der bereits im Ziel-Anzeigefenster vorhandenen 
                  Fenster-Knoten der Selektion wird im Ziel-Anzeigefenster nicht
                  verändert.}
	  \end{itemize}
      \item Copy Selection Changing Existing Layout\\
           Diese Funktion arbeitet genauso wie \gq{Copy Selection Keeping Existing
	   Layout}, nur wird das Layout der im Ziel-Anzeigefenster bereits vorhandenen
	   Fenster-Knoten gemäß dem ausgewählten Layout neu berechnet.
      \item Delete Selection\\
	   ...
	   Waren Fenster-Knoten und Fenster-Kanten der gelöschten Selektion
           ausgeblendet (siehe Funktion \ref{Selektionen ausblenden}), so
      ...
      \item Selection Set Operation\\
	    ...
	    \begin{itemize}
	       \item{Der Benutzer startet die Funktion über durch Auswahl des
	       ...
	       \item{Der Benutzer wählt dort die beiden Quell-Selektionen (LEFT\_SOURCE und 
                     RIGHT\_SOURCE) aus, bestimmt die durchzuführende Mengenoperation und
		     gibt unter TARGET den Namen der neu zu erzeugenden Selektion 
                     ein. Dann bestätigt er die Eingabe mit OK.}
	       \item{GIANT führt die Mengenoperation aus.}
	    \end{itemize}
  ...
  Eintrag \gq{Fit in Window}, welcher dafür sorgt, daß der gesamte Graph im
  ...
  Mittels \gq{Pick Edge} kann Scrolling und Zoomstufe automatisch so eingestellt werden,
  daß eine ausgewählte Kante komplett sichtbar ist, dazu wird \gq{Pick Edge} und
  danach die auszuwählende Kante angeklickt.
...
\label{Fadenkreuzcursor}
        ...
        Funktionen (siehe z.B. Funktion \ref{Platz schaffen}) in ein Fadenkreuz.
