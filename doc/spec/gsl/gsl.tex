% =============================================================================
%  $RCSfile: gsl.tex,v $ $Revision: 1.4 $
%  $Date: 2003/04/24 02:08:31 $
%  $Author: keulsn $
%
...
Aktivierungsumgebung \bottomenv. Dann wertet der GSL Interpreter den Ausdruck
aus, der in der Datei \gq{standard.gsl} gespeichert ist. Dieser Ausdruck
...
Ein Objekt ist jedoch nicht festgelegt auf Werte eines bestimmten Typs.
Jedes Objekt kann einen Wert jedes Typs speichern.
...
Der Ausdruck aus der Datei \gq{standard.gsl} wird ausgewertet.
Dieser Ausdruck kann z.B.\ neue
...
\ref{standard} beschrieben wird. Durch Änderungen an der Datei
\gq{standard.gsl} kann die Sprachumgebung an neue Anforderungen angepasst
werden. Die Implementierung des GSL Interpreters muss dazu nicht bekannt sein.
...
sichtbar, die in irgendeiner Aktivierungsumgebung $A_2$ mit $A_2 \le A_1$
sichtbar sind.
...
ihrer korrekten Reihenfolge und sonst nichts enthält.
\end{enumerate}
...
Zugriffspfad auf das Skript $S$.
  ...
  in der Aktivierungsinformation angegebene zulässige Aktivierungsumgebung.
  ...
  \item Die \nexpression wird ausgewertet und hat den Wert $R$ als Ergebnis.
  \label{script_result}
...
Objekte, in denen die Aktivierungsinformation gespeichert ist. Vor der
Auswertung des Ausdrucks in der Datei \gq{standard.gsl} erzeugt der GSL
Interpreter in der Aktivierungsumgebung \bottomenv die hier genannten
Variablen.
...
Scripts der Bibliothek enthalten.}
\end{scriptdef}
...
ausgewertet unabhängig davon, ob $b$ erfüllt ist, oder nicht.}
\end{scriptdef}
...
\sparam{b}{\tnatural bzw.\ \tnodeid bzw.\ \tedgeid}{Abhängig von dem Typ
von a ebenfalls eine Zahl oder der entsprechende Elementtyp}
...
\saction{Es werden zwei verschiedene Verwendungen unterschieden:
\begin{enumerate}
\item a ist vom Typ \tnatural und b ist vom Typ \tnatural. Dann hat
dieses Script die Summe der beiden Zahlen als Ergebnis. Es ist ein
...
ein Objekt, dessen Wert $N$ vom Typ \tnodeset ist. Dann muss b vom Typ
\tnodeid sein. Der Wert $N$ wird um das Element b vergrößert, falls b zuvor
noch nicht in $N$ enthalten war, andernfalls wird nichts getan. Das Ergebnis
ist \toknull.
\item a ist vom Typ \tvarreference und hat als Wert einen Zugriffspfad auf
ein Objekt, dessen Wert $E$ vom Typ \tedgeset ist. Dann muss b vom Typ
\tedgeid sein. Der Wert $E$ wird um das Element b vergrößert, falls b zuvor
noch nicht in $E$ enthalten war, andernfalls wird nichts getan. Das Ergebnis
ist \toknull.
\end{enumerate}}
\end{scriptdef}
...
\item a ist vom Typ \tvarreference und hat als Wert einen Zugriffspfad auf
ein Objekt, dessen Wert $N$ vom Typ \tnodeset ist. Dann muss b vom Typ
\tnodeid sein. Der Wert $N$ wird um das Element b verkleinert, falls b zuvor
in $N$ enthalten war, andernfalls wird nichts getan. Das Ergebnis
ist \toknull.
\item a ist vom Typ \tvarreference und hat als Wert einen Zugriffspfad auf
ein Objekt, dessen Wert $E$ vom Typ \tedgeset ist. Dann muss b vom Typ
\tedgeid sein. Der Wert $E$ wird um das Element b verkleinert, falls b zuvor
in $E$ enthalten war, andernfalls wird nichts getan. Das Ergebnis
ist \toknull.
\end{enumerate}}
\end{scriptdef}
...
ist. Der Name einer IML-Kanten hat das Format \gq{Knotentyp.Attributname}.
...
\sparam{set}{\tnodeset oder \tedgeset}{Ein Wert eines der Mengentypen}
...
ist, so muss element ein Wert vom Typ \tedgeid sein.}
\sresult{\tboolean}{\toktrue, falls element in set enthalten ist, \tokfalse
sonst}
\saction{Testet, ob element in set enthalten ist.}
\end{scriptdef}
...
\sparam{obj}{\tlist oder \tnodeset oder \tedgeset}{Irgendeine Liste, oder
eine Menge}
...
\sresult{\tnodeset}{Menge, die den Wurzelknoten des IML-Graphen enthält und
nichts sonst.}
\saction{Hat als Ergebnis eine Menge, die nur den Wurzelknoten des IML-Graphen
enthält.}
\end{scriptdef}
...
\sparam{window\_name}{\tstring}{Name eines Anzeigefensters}
\sparam{layout\_algo}{\tstring}{Aufrufstring eines Layoutalgorithmus}
\sparam{add\_content}{\tobjectset}{Einzufügender IML-Teilgraph}
\sresult{\tnatural}{Anzahl der eingefügten IML-Knoten und IML-Kanten}
...
Es ist ein Laufzeitfehler, falls layout\_algo kein korrekter Aufrufstring ist.
Es ist ein Laufzeitfehler, falls kein Anzeigefenster mit Namen window\_name
existiert.}
\end{scriptdef}
...
\sresult{\tnatural}{Anzahl der entfernten IML-Knoten und IML-Kanten}
...
parameterloses Script, dass den boolschen Wert zum Ergebnis hat}
...
Script.}
\sresult{}{\toknull}
\saction{Wertet abwechselnd zuerst b und dann c aus. Stoppt, sobald b das
Ergebnis \tokfalse hat.}
\end{scriptdef}
...
parameterloses Script das als Ergebnis einen Wert vom Typ \tboolean hat.}
\sresult{}{\toknull}
\saction{Wertet c einmal aus. Wertet dann abwechselnd zuerst until und dann
c aus. Stoppt, sobald until das Ergebnis \tokfalse hat.}
\end{scriptdef}
