% ==============================================================================
%  $RCSfile: selection.tex,v $, $Revision: 1.11 $
...
Siehe hierzu auch Abschnitt \ref{Aktuelle Selektion vs Selektionen}. 
...
Anzeigefensters (siehe hierzu auch 
auch \ref {hervorheben von Knoten und Kanten}).
  ...
  \begin{proc}    
    ...
    \gq{Highlight Selection  Color 1 (2,3)} aus.
...
Anzeigefensters (nicht zum Kopieren in ein anderes Anzeigefenster).
  ...
  \begin{proc}    
    ...
    und wählt aus dem PopUp Menü den Eintrag \gq{Copy Selection}.
...
unverändert.
  ...
  \begin{proc}    
    ...
    Selektion in der Selektionsauswahlliste \ref{Selektionsauswahlliste}
    und wählt aus dem PopUp Menü den Eintrag \gq{Delete Selection} aus.
...
(siehe hierzu \ref{Selektion aus IML-Teilgraphen ableiten}).
  ...
  \begin{proc} 
    ...
    den Eintrag \gq{Create Window Selection from IML Subgraph} aus.
     
    \step[2]
    GIANT zeigt den allgemeinen Texteingabedialog \ref{DIALOG-WINDOW}.
      	    
    \step[3]
    Der Benutzer gibt einen Namen für die neu zu erstellende Selektion ein
    und bestätigt mit OK (gültige Namen siehe \ref{afa Zulaessige Namen}).
  ...
  \begin{aproc}
    ...
    \astep{4} Der Benutzer bricht den UseCase durch einen Rechtsklick ab.
  \end{aproc}
...
Zusätzlich zu den Möglichkeiten der Anfragsprache GQSL (siehe
\ref {GIANT Query Skripting Language}) kann der Benutzer
