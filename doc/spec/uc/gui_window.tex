% ==============================================================================
%  $RCSfile: gui_window.tex,v $, $Revision: 1.22 $
%  $Date: 2003/03/28 18:37:17 $
%  $Author: schwiemn $
%
...
Projektes anlegen.
  ...
  \begin{proc}    
    \step[1]
    Der Benutzer startet den UseCase über das PopUp 
     Menü \ref{WINDOW-LIST-POPUP}
    durch Auswahl von \gq{New Window}
    
    \step[2] 
    GIANT öffnet den allgemeinen Texteingabedialog 
    \gq{Enter Name for new Window} (siehe \ref{DIALOG-WINDOW}).
...
gehen hierbei unwiederbringlich verloren.
  ...
  \begin{proc}    
    ...
    \ref{Sicherheitsabfrage})
    und fragt nach, ob das
...
Dient zum Öffnen eines Anzeigefensters des Projektes.
  ...
  \begin{postfail}
    ...
    \cond 
    Es sind keine Fehlerfälle vorgesehen.
...
Näheres zur Persistenz von Anzeigefenstern ist unter
Abschnitt \ref{Project Persistenz der Projekte} spezifiziert.
...
insbesondere \ref{Einfügen von IML-Teilgraphen in Anzeigefenster}.
  ...
  \begin{proc}    
    ...
    semantischen Layouts gibt er in über den Layoutalgorithmen Dialog
    (siehe \ref{Layoutalgorithmen-Dialog}) auch die Kantenklassen vor,
    die für das Layout berücksichtigt werden sollen (Details
    zu Layoutalgorithmen siehe Kapitel \ref{Layoutalgorithmen}.
  ...
  \begin{aproc}
    \astep{4} Der Benutzer bricht den UseCase Eingabe mit Cancel ab.
...
siehe hierzu  \ref{Verhalten beim Einfügen von IML-Teilgraphen 
und Selektionen in Anzeigefenster} und
\ref{Einfügen von Selektionen in Anzeigefenster}).  
  ...
  \begin{postsuccess}
    ...
    sind auch im \gq{Ziel-Anzeigefenster} unter Beibehaltung des
  ...
  \begin{proc}    
    \step[1]
    Der Benutzer startet den UseCase über das PopUp Menü
    \ref{PopUp Menü Subgraph List} in der Subgraph Liste im Hauptfenster.
    ...
    Der Benutzer gibt gibt über das Fadenkreuz die Position vor, 
    an der die neuen Fenster-Knoten und Fenster-Kanten eingefügt werden 
    sollen (siehe \ref{Fadenkreuzcursor}).
    Die Statuszeile im Hauptfenster schaltet dannach auf Normalmodus zurück.
    ...
    Fenster-Knoten der Quell-Selektion folgendes:
    \begin {enumerate}
       \item
       Falls \gq{Copy Selection keep existing layout} gewählt wurde, wird
       ihre Position im Ziel-Anzeigefenster nicht verändert.
       
       \item 
       Falls \gq{Copy Selection change existing layout} gewählt wurde,
       ihre Position im Zielanzeigefenster gemäß dem Layout der
...
Mittels dieses UseCases können alle Fenster-Knoten und Fenser-Kanten
einer Selektion aus einem Anzeigefenster gelöscht werden
(siehe hierzu auch \ref{Verhalten beim Entfernen von Fenster-Knoten und 
Fenster-Kanten}).
  ...
  \begin{proc}    
    ...
    \step[1]
    Der User führt einen Rechtsklick auf die entsprechende Selektion in der
    Selektionsauswahlliste durch (siehe \ref{Selektionsauswahlliste})
    und wählt im entsprechenden PopUp Menü den Eintrag \gq{Delete Selection}
    aus.
    ...
    wirklich löschen soll (\gq{Really delete Selection xy from its window 
...
\index{Visualisierungsstil!ändern}
...
Visualisierungsstile (siehe auch \ref{Config Visualisierungsstile}) ändern.
  ...
  \begin{precond}
    ...
    \cond
    Es gibt mindestents einem vom Benutzer definierten Visualisierungsstil
    (siehe \ref{Config Visualisierungsstile}). 
  ...
  \begin{proc}    
    ...
    \step[1]
    Der Benutzer ändert den Visualisierungsstiel eines Anzeigefensters
    ...
    \step[2]
    GIANT ändert die Dartellung von Fenster-Knoten und Fenster-Kanten
    entsprechend ab.
...
Verändert die Position des sichtbaren Anzeigeinhaltes.
  ...
  \begin{proc}    
    ...
    \begin {enumerate}
      ...
      und vertikalen Scrollleisten des Anzeigefensterts (siehe    
...
Verändert den Maßstab der Darstellung von Knoten und Kanten (relativ).
  ...
  \begin{postsuccess}
    ...
    Der Detaillierungslevel (siehe \ref{Visualization Detailstufen})
    wurde ggf. automatisch angepaßt.
  ...
  \begin{proc}    
    ...
    \begin {enumerate}
      \item
      Der Benutzer gibt in der Zoomkontroll-Combobox 
      ...
      ändert den Zoomwert in festgelegten Schritten mit den Zoom+ oder 
      Zoom- Buttons. 
      
      \item
      Der Benutzer klickt auf den Button \gq{Display} der Zoomkontrolle
      (siehe \ref{GUI Zoom-Kontrolle}).
      
    \end {enumerate}
    
    \step[2]
    GIANT berechnet den neuen sichtaberen Anzeigeinhalt anhand der neuen
    Zoomstufe.
...
Wählt die Zoomstufe und scrollt so, daß eine gesamte Selektion im Fenster 
sichtbar ist.
...
Wählt die Zoomstufe und scrollt so, daß der gesamte Fensterinhalt im Fenster 
sichtbar ist.
  ...
  \begin{postsuccess}
    ...
    \cond
    Der angezeigte Bereich des sichtbaren Anzeigeinhalts wurde so
    vergrößert oder verkleinert, daß der gesamte Anzeigeinhalt im sichtbaren
    Anzeigeinhalt dargestellt ist, ggf. wurde auch entsprechend gescrollt.
    Der Detaillierungslevel (siehe \ref{Visualization Detailstufen}) 
    wurde ggf. automatisch angepaßt.
...
Wählt die Zoomstufe und Scrollt so, daß eine bestimmte Kante komplett im 
sichtbaren Anzeigeinhalt dargestellt ist.
  ...
  \begin{postfail}
    \cond Hat der Benutzer den UseCase abgebrochen,
          so werden keine Änderungen an der Zoomstufe vorgenommen.
  \end{postfail}
  ...
  \begin{proc}    
    ...
    \step[1]
    Der Benutzer Klickt den Button \gq{Zoom Edge} in der Zoomkontrolle
    (siehe \ref{GUI Zoom-Kontrolle}).
    ...
    der Fadenkreuzcursor (siehe \ref{Fadenkreuzcursor}) wird
  ...
  \begin{aproc}
    ...
    auf eine beliebige Stelle -- halt nicht auf eine Kante --
    klickt oder einen Rechtsklick mit der Maus ausführt.
...
% ==============================================================================
\begin{uc}[Label]{UC: Verschieben von Fenster-Knoten und Selektionen mittels
                  Cut and Paste}
...
\gq{Cut and Paste}.
  ...
  \begin{proc}    
    ...
    \begin{enumerate}
      \item
      Einen Rechtsklick auf eine Selektion in der Selektionsauswahliste
      durchführt (siehe \ref{Selektionsauswahlliste}) und im
      Popup-Menü den Eintrag \gq{Move Selection} auswählt,

      \item
      oder einen Rechtsklick auf einen Fenster-Knoten durchführt
      und im PopupMenü (siehe \ref{Node-Popup-Menü}) den Eintrag
      \gq{Move Node} auswählt.

    \end{enumerate}

    \step[2]
    GIANT geht in den den \gq{Paste Modus} über und zeigt dies in 
    der Statusleiste (siehe \ref{Statuszeile}) des Hauptfensters an.
    Der Cursor wird, falls er über den Anzeigeinhalt eines Anzeigefensters
    bewegt wird, zum Fadenkreuz (siehe \ref{Fadenkreuzcursor}).
    Die Funktionalität zum Zoomen und Scrollen des Anzeigefensters mittels
    der beiden UseCases \ref{Anzeigefenster scrollen} und
    \ref{Anzeigefenster zoomen} bleibt weiterhin verfügbar, alle
    andere Funktionalität von GIANT wird gesperrt. 
    Der Benutzer kann insbesondere das aktuelle Anzeigefenster 
    (in welchem er den UseCase gestartet -- also \gq{Cut} ausgeführt hat --
    nicht verlassen).

    \step[3]
    Der Benutzer klickt mit dem Fadenkreuzkursor (linke Maustaste)
    an eine bliebige Stelle innerhalb des sichtbaren Anzeigeinhaltes
    des Anzeigefensters.

    \step[4]
    GIANT verschiebt die betroffenen Fenster-Knoten an die gewünschte
    stelle.
  ...
  \begin{aproc}
    ...
    \astep{3} Der Benutzer kann den UseCase abbrechen, indem er
              einen Rechtsklick mit der Maus auf eine beliebige Stelle 
              durchführt.
  \end{aproc}
...
% ==============================================================================
\begin{uc}[Label]{UC: Verschieben einzelner Fenster-Knoten mittels 
                      Drag and Drop}
\index{verschieben!einzelne Knoten}
\index{verschieben!ganze Selektionen}
...
Drop auf dem sichtbaren Anzeigeinhalt verschoben werden.
  ...
  \begin{postsuccess}
    ...
    \cond
    Der einzelne Fensterknoten wurden im Anzeigeinhalt verschoben.  
  \end{postsuccess}
