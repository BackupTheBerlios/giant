% ==============================================================================
%  $RCSfile: gui_window.tex,v $, $Revision: 1.11 $
%  $Date: 2003/02/24 11:01:00 $
%  $Author: birdy $
%
...
insbesondere \ref{Einfügen von Selektionen in Anzeigefenster}.
  ...
  \begin{proc}    
    ...
    ES GIBT ZWEI MENÜEINTÄGE \\
    - KOPIEREN UNTER BEIBEHLATUNG des Layouts bereits vorhandenen Knoten\\
    - Kopieren bei voller Übernahme des Layouts
    <<<<
...
\ref{Verhalten beim Entfernen von Fenster-Knoten und Fenster-Kanten}.
  ...
  \begin{proc}    
    ...
    \step[1]
    Der UseCase führt einen Rechtsklick auf die entsprechende Selektion durch
    <<<<VERWEIS GUI - Liste der Selektionen>>>>> und wählt im 
    entsprechenden PopUp Menü den Eintrag
...
Verändert die Position des sichtbaren Anzeigeinhaltes.
  ...
  \begin{proc}    
    ...
    \begin {enumerate}
      ...
      <<<<<<<<<<<<<<<<VERWEIS GUI>>>>>>>>>>>>>>>>>>
      <<<<<<<<<<<<<SCROLLLEISTEN IN DER GUI Berücksichtigen>>>>>>>>>>>>>>
...
können. So kann der Benutzer an einer bliebigen Stelle des Anzeigefensters
...
Siehe hierzu auch \ref{Auseinanderschieben von Fenster-Knoten}.
  ...
  \begin{proc}    
    \step[1]
    Der Benutzer startet den UseCase über das PopUp Menü
    >>>>>>>>>>>>>>>VERWEIS PopUp Menü welches beim Rechtsklick
    innerhalb eines ANzeigefensters erscheint <<<< >>>Verweis auf
    ...
    sollen über das Fadenkreuz vor.
    >>>>VERWEIS GUI Fadenkreuz>>>>>
    ...
    Betrag die Fenster-Knoten auseinander geschoben werden sollen.
        >>> DIALOG ZUR EINGABE DES BETRGES UM DEN GESCHOBEN WERDEN SOLL MUSS
    NOCH IN GUI BESCHREIBUNG >>>>>>>>>>>>>>>>>>.
   
    \step[4]
    Der Benutzer wählt einen geeigneten Betrag aus.
 ...
 \begin{precond}
  ...
  \begin{postsuccess}
    ...
    In der Liste über die Pins des Anzeigefensters
     <<<<<VERWEIS GUI>>>> befindet sich
    ein neuer Pin mit dem entsprechenden Namen.
  ...
  \begin{proc}    
    ...
    Menü den Eintrag \gq{Pin erstellen} aus <<<<<VERWEIS GUI>>>>>.
 ...
 \begin{precond}
  ...
  \begin{proc}    
    ...
    \begin{enumerate}
      ...
      in der Liste aus.
      >>>>>VERWEIS GUI - LISTE DER PINS In den Anzeigefenstern<<<<<<
      
      \item
      Der Benutzer öffnet das entsprechende PopUp Menü durch
      rechtsklick auf den Pin und wählt den Eintrag \gq{Fokus Pin}
      aus.
      <<<<<<<<<<<<<<VERWEIS GUI>>>>>>>>>>>>>>><
...
Löscht einen Pin.
 ...
 \begin{precond}
  ...
  \begin{postsuccess}
    \cond 
    Der Pin ist gelöscht und nicht mehr in der Liste des entsprechenden
  ...
  \begin{proc}    
    ...
    auf den Pin und wählt den Eintrag \gq{Pin löschen} aus.
    <<<<<VERWEIS GUI>>>>>>>>>>>
