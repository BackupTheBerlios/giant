% ==============================================================================
%  $RCSfile: gui.tex,v $, $Revision: 1.94 $
...
Im Hauptfenster (Main Window, \ref{Main-Window-Pic}) werden die
Anzeigefenster und
...
Subgraph List (4 Spalten).
  ...
  \label{GUI Subgraph List}
    ...
    \subsubsection {Inhalt Subgraph List} 
      ...
      In der Spalte Name stehen die Namen der existierenden Subgraphen,
      in Nodes die Anzahl der Graph-Knoten des Subgraphen, in Edges die
      Anzahl der Graph-Kanten und in Highlighted befindet sich
      ein Kästchen in der
...
\index{Anzeigefenster}
  ...
  Im Programm kann es mehrere Anzeigefenster geben. 
  Ein Anzeigefenster (\ref{Graph-Window-Pic}) besteht
 ...
 \ref{Verschieben des sichtbaren Anzeigeinhaltes über die Minimap}).
  ...
  \subsection{Stilauswahl-Combobox}
    ...
    Visualisierungsstil ausgewählt werden.
...
\section{Knoten-Informationsfenster} \label{Knoten-Informationsfenster}
    ...
    zu Fenster-Knoten angezeigt werden.
    ...
    \begin{enumerate}
      ...
      \item Class (die Kantenklasse der abgehenden Kante)
    \end{enumerate}    
    ...
    \subsection{Liste Predecessor Nodes}
      ...
      \begin{enumerate}
        ...
        \item Class (die Kantenklasse der eingehenden Kante)
      \end{enumerate}
...
Der Skriptdialog \gq{Query Dialog} hat zuoberst ein Textfeld \gq{Query Text},
in ein GSL Skript eingegeben werden kann.
...
Der Set-Operation-Dialog (\ref{Set-Operation-Dialog-Pic}) dient dazu, Mengenoperationen über 
Selektionen und
...
Der Layoutalgorithmen Dialog (\ref{Layout-Algorithm-Dialog-Pic}) bietet folgende Möglichkeiten:

\begin {enumerate}
 \item Auswahl des Layoutalgorithmuses
 \item Ggf. Anzeige der zu layoutenden Selektion
 \item Ggf. Eigabefeld für die ID des Wurzelknotens bei Treelayouts
...
Im Dialogfenster Make Room (\ref{Make-Room-Dialog-Pic}) wird in einem Textfeld als Zahl 
eingegeben, um wie viel
Pixel die Fenster-Knoten an einer vorher markierten Stelle auseinandergeschoben werden sollen.
...
Ein allgemeiner Fehlerdialog (\ref{Error-Window-Pic}) ist ein Fenster \gq{Error Window}
mit dem Titel \gq{Error}. Im Fenster befindet sich ein Label zur
Ausgabe der Fehlermeldung und ein Button mit der standardmäßigen Beschriftung \gq{OK}. 
...
Eine allgemeine Sicherheitsabfrage (\ref{Confirmation-Window-Pic}) ist ein Fenster 
(\gq{Confirmation Window}) mit dem
...
des Standard-Filechooser-Dialogs. Dieser Dialog ist Bestandteil der
Widget-Bibliothek von GTK.
...
\label{Progressbar-Runs}
     ...
     dass das System noch arbeitet (beispielhafte Grafik siehe \ref{Progressbar-Runs-Pic}).
...
\label{Progressbar-Modale}      
     ...
     \end{figure}
     Der \gq{Progressbar-Modal} ist ein Fenster (\ref{Progressbar-Modal-Pic}), 
     dass über den aktuellen Fortschritt
     einer Berechnung informiert, ohne dabei den Gesamtaufwand zu 
     kennen. Hierzu wird kontinuierlich die Anzahl der schon bearbeiteten
     ...
     der die momentane Berechnung abbricht.
