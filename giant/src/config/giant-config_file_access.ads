...
--
-- $RCSfile: giant-config_file_access.ads,v $, $Revision: 1.1 $
-- $Author: schwiemn $
-- $Date: 2003/06/20 15:28:01 $
--
-- -----
-- This package holds the functionality needed to access and handle
-- xml configuration files used for giant.
-- 
-- Its implemented quite independent from giant itself.
--
...
package Giant.Config is
   ...
   -- Only used to build the array "Default_Values"
   ...
   type Default_Values_Array is array (integer range <>)
   ...
   -- values will be used.
   Default_Values : constant Default_Values_Array :=
     ( (To_UStr ("Actual_Selection_Highlight_Color"), "RGB:AA/AA/AA"),
       (To_UStr ("Selection_Highlight_Color_1"), "RGB:AA/AA/AA"),
       (To_UStr ("Selection_Highlight_Color_2"), "RGB:AA/AA/AA"),
       (To_UStr ("Selection_Highlight_Color_3"), "RGB:AA/AA/AA"),
       (To_UStr ("IML_Subgraph_Highlight_Color_1"), "RGB:AA/AA/AA"),
       (To_UStr ("IML_Subgraph_Highlight_Color_2"), "RGB:AA/AA/AA"),
       (To_UStr ("IML_Subgraph_Highlight_Color_3"), "RGB:AA/AA/AA"));
   ...
   -- "Resources_Directory"
   --
   -- If this setting is not definded in
   -- a config file (or if the file is not found) then the 
   -- "actual working directory of the current execution environment 
   -- will be returned".
   
   -- "Icon_For_Node_Annotations" 
   --
   -- A Reletive Path will be resolved:
   -- 1. Check whether the file is found relative to the dirctory of the
   --    config file. If Not:
   -- 2. Check whether the file is found relative towards
   --    Resources_Directory
   --
   -- If this setting is not defined in a config file or the Icon is not 
   -- found a null pointer will be returned when demanding an 
   -- null pointer will be returned when asking for the image.
   ...
   --     exist.
   --     ONLY A ABSOLUTE PATH for "GIANT_Config_File" WILL WORK.
   --
   ...
   --      ONLY A ABSOLUTE PATH for "User_Config_File" WILL WORK.
   --
   -- Raises:
